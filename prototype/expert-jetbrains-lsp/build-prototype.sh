...
prototype_plugin_root="$prototype_root/build/plugin/spellixir-expert-prototype"
prototype_distribution="$prototype_root/build/distributions/spellixir-expert-prototype.zip"

if [[ ! -x "$prototype_compiler" || -z "$prototype_jar" ]]; then
  echo "Set IDEA_HOME to a Host IDE installation containing Kotlin, and ensure a JDK jar tool is on PATH."
  exit 1
fi
...
"$prototype_compiler" \
  "$prototype_root/src/main/kotlin/com/cimere/spellixir/prototype/ExpertIntegrationProvider.kt" \
...
"$prototype_jar" --create \
  --file "$prototype_plugin_root/lib/spellixir-expert-prototype.jar" \
  -C "$prototype_classes" . \
